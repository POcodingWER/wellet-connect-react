...
function App() {
  ...
  window.klaytn.on('accountsChanged',function(accounts){    //change event 함수
  ...
  const getlog = async ()=>{
    ...
    console.log(getBlockEvent); //전체 목록

    const checkOfTokenOwner =await getBlockEvent.filter(async(v)=>{
    const findOwner =  await contract.methods.ownerOf(v.returnValues.tokenId).call(); 
    return(findOwner.toLowerCase() === window.klaytn.selectedAddress.toLowerCase())
  })
    // await getBlockEvent.map(async(v)=>{
    //   console.log(v.returnValues.tokenId)
    //   console.log(checkOfTokenOwner)
    //   box.push(v.returnValues.tokenId)
  ...
  const ttttttte = ()=>{
    let checkOfTokenOwner= [1,1,1,1,1,1,2,2,3,3].filter((v)=>{return v===2})
    console.log(checkOfTokenOwner);
  }
  ...
  return (
    <div className="App">
      <header className="App-header">
        <button style={{width:'100px',height:'100px'}} onClick={getlog}>지갑연결 </button>
